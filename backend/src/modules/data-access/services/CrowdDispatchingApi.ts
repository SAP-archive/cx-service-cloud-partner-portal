...
export class CrowdDispatchingApi {
  ...
  public static async get<T>(userData: UserData, path: string, queryString?: Object): Promise<CrowdDispatchingApiResponse<T> | undefined> {
    return this.makeRequest<T>('GET', userData, path, queryString);
  }

  public static async put<T>(userData: UserData, path: string, data: T, queryString?: Object, useFormData?: boolean): Promise<CrowdDispatchingApiResponse<T> | undefined> {
    return this.makeRequest<T>('PUT', userData, path, queryString, data, useFormData);
  }

  public static async post<T>(userData: UserData, path: string, data: any, queryString?: Object, useFormData?: boolean): Promise<CrowdDispatchingApiResponse<T> | undefined> {
    return this.makeRequest<T>('POST', userData, path, queryString, data, useFormData);
  }

  public static async delete<T>(userData: UserData, path: string, data?: T, queryString?: Object, useFormData?: boolean): Promise<CrowdDispatchingApiResponse<T> | undefined> {
  ...
  public static async makeRequest<T>(
    ...
    data?: any,
    useFormData?: boolean,
  ): Promise<CrowdDispatchingApiResponse<T> | undefined> {
    const pathSeparator = (path.indexOf('/') === 0) ? '' : '/';
    return HttpClientService.send<CrowdDispatchingApiResponse<T>>({
      method,
      path: `${CrowdDispatchingApi.API_PATH}${pathSeparator}${path}`,
      queryString,
      data,
